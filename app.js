...
const CLASS_HINTS = Object.freeze({
  ...
  heretic: [
    ...
    { title: 'Пакт силы', text: 'Пакт силы сначала даёт +2 к Силе, а потом удваивает итог. При нулевой Порче это даёт +4, а любое итоговое значение выше +5 ограничивается до +5.' },
...
function effectiveStatValue(card, stat, taint = Team.taint) {
  ...
  if (id !== 'heretic') return Math.min(5, value);

  const pact = selectedHereticPact();
  if (pact === 'pride') value += 1;
  if (pact === 'strength' && stat === 'strength') value = (value + 2) * 2;

  /* Жёсткий предел системы: никакая характеристика не может быть выше +5.
     Ограничение применяется после всех бонусов, Порчи и пактов. */
